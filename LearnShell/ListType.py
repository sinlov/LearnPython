...
for_sort.sort()
print("list for sort before", for_sort)
# python 对于未指定的混合序列，排序是不定的，默认顺序是，先对所有数字进行排序，然后对于字符串按照字典排序，如果是对象类型，则非常混乱
# 列表中，内置函数， sort() append() insert 都是直接对对象进行修改，没有返回值。所以，直接打印 list.soft() 会返回none。
# 而字符串的操作函数，如 upper() 是返回一个字符串（其实是包含一个全部是大写的字符串拷贝，所以未对原有字符串对象进行修改，新增了一个字符串）
# 如果是希望得到一个排好序的拷贝，使用 sorted() 或者 reversed, 分别意思为接受一个列表作为参数并返回一个排好序的或者倒序的拷贝
